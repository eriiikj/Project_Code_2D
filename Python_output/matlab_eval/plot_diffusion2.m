...
scatter(bccoords(:,1),bccoords(:,2),1e18*abs(rr)+0.01,1e17*rr+0.01,'filled')
colorbar
box on
axis equal
...
figure()
hold on
s = find(abs(jint4)>mean(abs(jint4))*2);
jint4(s) = 0;
bccoords = coordTG(indNodBd,:);
...
hold on
bccoords = coordTG(indNodBd,:);
scatter(bccoords(:,1),bccoords(:,2),1e13*abs(grainArr(g).jint)+0.01,1e13*(grainArr(g).jint),'filled')
colorbar
box on
axis equal
title('Computed jint')
