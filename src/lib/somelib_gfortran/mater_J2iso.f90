...
! M. Wallin 2011-05-17
!  - functionality J2iso_getVal added
! M. Ristinmaa 2011-05-19
!  - Changed arguments J2iso_getVal J2iso_init
...
interface J2iso_accept
  module procedure accept_J2_iso1
  module procedure accept_J2_iso2
  module procedure accept_J2_iso3
end interface
private accept_J2_iso1


interface dJ2iso
  module procedure dJ2_iso_specificGP
  module procedure dJ2_iso_specificGP_vol
  module procedure dJ2_iso0
  module procedure dJ2_iso1
  module procedure dJ2_iso2
  module procedure dJ2_iso1_up
end interface
private dJ2_iso0, dJ2_iso1, dJ2_iso2


interface J2iso
  module procedure J2_iso_specificGP
  module procedure J2_iso_specificGP_vol
  module procedure J2_iso0
  module procedure J2_iso1
  module procedure J2_iso2
  module procedure J2_iso1_up
end interface
...
subroutine J2iso_getValScalar(stype,out)
  implicit none
  integer                         :: ierr
  character(len=*)                :: stype
...
subroutine J2iso_getValVector(stype,out)
  implicit none
  integer                         :: ierr
  character(len=*)                :: stype
  double precision, allocatable   :: out(:,:,:)
...
subroutine init_J2_iso1(mp,Nelm,Ngp)
  implicit none
  integer                         :: Nisv, Ngp, Nelm, ierr
  double precision                :: mp(6)

  print *, 'Initiating J2 iso'
  Nisv=9
...
subroutine change_J2_mp(mp)
  implicit none
  double precision, intent(in) :: mp(6)

  ep=mp

  return
end subroutine change_J2_mp


subroutine accept_J2_iso1
  implicit none
...
! in the first iteration is calculated at the n-1 state
  ...
  return
end subroutine accept_J2_iso1

! one gauss point per element
subroutine accept_J2_iso2(dg)
  implicit none
  double precision                :: dg(:,:)
...
! in the first iteration is calculated at the n-1 state
  ...
  return
end subroutine accept_J2_iso2

! more than one gauss point per element
subroutine accept_J2_iso3(dg)
  implicit none
  double precision                :: dg(:,:,:)
...
! in the first iteration is calculated at the n-1 state
  ...
  return
end subroutine accept_J2_iso3


subroutine J2_iso1_up(stype,stress,ef_New,th,ie,mp)
  implicit none
  ...
  do gp=1,nr_gp
    call J2_iso2('Cauchy', stress(:,gp), Isv(:,gp,ie,old), Isv(:,gp,ie,new), & 
                 ep, ef_New(:,gp) ,ie,gp,mp)
  ...
  return
end subroutine J2_iso1_up


! Calculate stress for specific gauss point in the element
subroutine J2_iso_specificGP(stype,stress,ef_New,ie,gp,mp)
  implicit none
  ...
  call J2_iso2(stype, stress(:), Isv(:,gp,ie,old), Isv(:,gp,ie,new), & 
               ep, ef_New(:),ie,gp, mp)

  return
end subroutine J2_iso_specificGP

! Calculate stress for a specific gauss point in the element and for volumen increase
subroutine J2_iso_specificGP_vol(stype,stress,ef_New,ie,gp,vol)
  implicit none
  double precision                :: stress(:), ef_New(:), dgep_33
  character(len=*)                :: stype
  integer                         :: gp, ie
  logical                         :: vol

  ! Call special dJ2 routine
  call J2_iso2_3d_to_2d(stype, stress(:), Isv(:,gp,ie,old), Isv(:,gp,ie,new), & 
                        ep, ef_New(:),ie,gp)
  return
end subroutine J2_iso_specificGP_vol

! One Gauss point per element
subroutine J2_iso0(stype,stress,ef_New,ie,mp)
  implicit none
  double precision                :: stress(:), ef_New(:)
  character(len=*)                :: stype
  integer                         :: ie
  ...
  call J2_iso2(stype, stress(:), Isv(:,1,ie,old), Isv(:,1,ie,new), & 
               ep, ef_New(:),ie,1,mp)
...
subroutine J2_iso1(stype,stress,ef_New,ie,mp)
  implicit none
  double precision                :: stress(:,:), ef_New(:,:)
  character(len=*)                :: stype
  integer                         :: gp, nr_gp,ie
  ...
  do gp=1,nr_gp
    call J2_iso2(stype, stress(:,gp), Isv(:,gp,ie,old), Isv(:,gp,ie,new), & 
                 ep, ef_New(:,gp),ie,gp,mp)
  enddo
...
subroutine J2_iso2(stype,stress,Isv_Old,Isv_New,ep,ef_New,ie,gp,mp)
  implicit none
  ...
  double precision                :: s_trial(3,3), n(3,3) , vm
...
subroutine J2_iso2_3d_to_2d(stype,stress,Isv_Old,Isv_New,ep,ef_New,ie,gp)
  implicit none
  double precision                :: stress(:), stress_tmp(6), Isv_Old(:), Isv_New(:)
  double precision                :: ep(:), ef_New(:)
  character(len=*)                :: stype  
  integer                         :: ie, gp
  ...
  stress  = (/S(1,1),S(2,2),S(3,3),S(1,2)/)
  return
end subroutine J2_iso2_3d_to_2d


subroutine dJ2_iso1_up(stype,D,ddwdjj,ef_New,th,ie,mp)
  implicit none
  character(len=*)                :: stype
  ...
  do gp=1,nr_gp
    F_m=getF(ef_New(:,gp))
    Ja=det3(F_m)
    call dJ2_iso2('ul',D(:,:,gp),ep,ef_New(:,gp), &
                  Isv(:,gp,ie,Old), Isv(:,gp,ie,New),ie,gp,mp)
  ...
  return
end subroutine dJ2_iso1_up


! Calculate D for a specific gauss point in that element
subroutine dJ2_iso_specificGP(stype,D,ef_New,ie,gp,mp)
  implicit none
  character(len=*)                :: stype
  ...
  call dJ2_iso2(stype,D(:,:),ep,ef_New(:), Isv(:,gp,ie,Old), Isv(:,gp,ie,New),ie,gp,mp)

  return
end subroutine dJ2_iso_specificGP

! Calculate D for a specific gauss point in that element and for volumen increase
subroutine dJ2_iso_specificGP_vol(stype, D, ef_New, ie, gp, vol)
  implicit none
  character(len=*)                :: stype
  double precision                :: D(:,:), ef_New(:), dgep_33
  integer                         :: gp, ie
  logical                         :: vol

  ! Call special dJ2 routine
  call dJ2_iso2_3d_to_2d(stype,D(:,:),ep, ef_New(:), &
                         Isv(:,gp,ie,Old), Isv(:,gp,ie,New),ie,gp)
  return
end subroutine dJ2_iso_specificGP_vol


! One gauss point per element
subroutine dJ2_iso0(stype,D,ef_New,ie,mp)
  implicit none
  character(len=*)                :: stype
  double precision                :: D(:,:), ef_New(:)
  integer                         :: gp,nr_gp, ie
  ...
  call dJ2_iso2(stype,D(:,:),ep,ef_New(:), &
                  Isv(:,1,ie,Old), Isv(:,1,ie,New),ie,1,mp)

  return
end subroutine dJ2_iso0


subroutine dJ2_iso1(stype,D,ef_New,ie,mp)
  implicit none
  character(len=*)                :: stype
  double precision                :: D(:,:,:), ef_New(:,:)
  integer                         :: gp, nr_gp, ie
...
subroutine dJ2_iso2(stype,Dout,ep,ef_New,Isv_Old,Isv_New,ie,gp,mp)
  implicit none
  character(len=*)                :: stype
  double precision                :: ep(:), ef_New(:)
...
subroutine test1_J21(a)
  implicit none

  integer :: a

  return
end subroutine test1_J21


subroutine dJ2_iso2_3d_to_2d(stype,Dout,ep,ef_New,Isv_Old,Isv_New,ie,gp)
  implicit none
  character(len=*)                :: stype
  double precision                :: ep(:), ef_New(:)
  double precision                :: Isv_New(:), Isv_Old(:), Dout(:,:), D_tmp(6,6)
  integer                         :: ie, gp
...
end subroutine dJ2_iso2_3d_to_2d


end module mater_J2iso
