...
subroutine VTKopen1(filename)
   ...
   character                :: buffer*80
      ...
      open(unit=ivtk,file=filename,access='stream',form='unformatted',convert='BIG_ENDIAN')
      ! File header
      ! ===========
...
!open(unit=ivtk,file=filename,form='binary',convert='BIG_ENDIAN')
      ...
      open(unit=ivtk,file=filee,access='stream',form='unformatted',convert='BIG_ENDIAN')
      ! File header
      ! ===========
...
!     end if
     ...
     if (.false..eqv.(cell_name.eq.name)) then
       buffer = 'SCALARS '//name//'  double'//lf                                             ;
       write(ivtk) trim(buffer)
       buffer = 'LOOKUP_TABLE default'//lf                                                   ;
       cell_name=trim(name)
