...
!  M_pqij C_ijkl M_stkl
   ...
   elseif ((size(var,1).eq.6).and.(size(var,2).eq.6)) then
    ...
    M6(4,:)=(/F(1,1)*F(2,1), F(1,2)*F(2,2), F(1,3)*F(2,3), F(1,1)*F(2,2)+ F(1,2)*F(2,1), &
              F(1,1)*F(2,3)+F(1,3)*F(2,1), F(1,2)*F(2,3)+F(1,3)*F(2,2) /)
    M6(5,:)=(/F(1,1)*F(3,1), F(1,2)*F(3,2), F(1,3)*F(3,3), F(1,1)*F(3,2)+ F(1,2)*F(3,1), &
              F(1,1)*F(3,3)+F(1,3)*F(3,1), F(1,2)*F(3,3)+F(1,3)*F(3,2) /)
    M6(6,:)=(/F(2,1)*F(3,1), F(2,2)*F(3,2), F(2,3)*F(3,3), F(2,1)*F(3,2)+ F(2,2)*F(3,1), &
              F(2,1)*F(3,3)+F(2,3)*F(3,1), F(2,2)*F(3,3)+F(2,3)*F(3,2) /)
...
!  M_pqij C_ijkl M_stkl
   ...
   elseif ((size(var,1).eq.6).and.(size(var,2).eq.6)) then
    ...
    M6(4,:)=(/Fi(1,1)*Fi(2,1), Fi(1,2)*Fi(2,2), Fi(1,3)*Fi(2,3), Fi(1,1)*Fi(2,2)+ Fi(1,2)*Fi(2,1), &
              Fi(1,1)*Fi(2,3)+Fi(1,3)*Fi(2,1), Fi(1,2)*Fi(2,3)+Fi(1,3)*Fi(2,2) /)
    M6(5,:)=(/Fi(1,1)*Fi(3,1), Fi(1,2)*Fi(3,2), Fi(1,3)*Fi(3,3), Fi(1,1)*Fi(3,2)+ Fi(1,2)*Fi(3,1), &
              Fi(1,1)*Fi(3,3)+Fi(1,3)*Fi(3,1), Fi(1,2)*Fi(3,3)+Fi(1,3)*Fi(3,2) /)
    M6(6,:)=(/Fi(2,1)*Fi(3,1), Fi(2,2)*Fi(3,2), Fi(2,3)*Fi(3,3), Fi(2,1)*Fi(3,2)+ Fi(2,2)*Fi(3,1), &
              Fi(2,1)*Fi(3,3)+Fi(2,3)*Fi(3,1), Fi(2,2)*Fi(3,3)+Fi(2,3)*Fi(3,2) /)
...
INTEGER                          :: KM, KN
DOUBLE PRECISION,PARAMETER       :: ZERO=0., ONE=1., THREE=3., ONETHIRD=ONE/THREE 
...
DOUBLE PRECISION, DIMENSION(3,3) :: C, EYE, T, A, Y, E, B, D,&
                                    F, M, N, INVT, INVY
INTEGER, DIMENSION(6,2)          :: IJ
DOUBLE PRECISION, DIMENSION(6,6) :: S
DOUBLE PRECISION                 :: S1, S2, S3, S4, S5
INTEGER                          :: KM, KN, I, J, K, L
!DOUBLE PRECISION                 :: 
DOUBLE PRECISION, PARAMETER      :: ZERO=0., ONE=1., TWO=2., THREE=3., FOUR=4., TWELVE=12., HALF=ONE/TWO, THIRD=ONE/THREE, &
...
INTEGER                          :: M,N
        ...
        M=1
        N=1
        OUT(1,:)=(/A(M,1)*B(N,1), A(M,2)*B(N,2), A(M,3)*B(N,3), A(M,1)*B(N,2)+ A(M,2)*B(N,1), A(M,1)*B(N,3)+A(M,3)*B(N,1), &
        A(M,2)*B(N,3)+A(M,3)*B(N,2) /)
        M=2
        N=2
        OUT(2,:)=(/A(M,1)*B(N,1), A(M,2)*B(N,2), A(M,3)*B(N,3), A(M,1)*B(N,2)+ A(M,2)*B(N,1), A(M,1)*B(N,3)+A(M,3)*B(N,1), &
        A(M,2)*B(N,3)+A(M,3)*B(N,2) /)
        M=3
        N=3
        OUT(3,:)=(/A(M,1)*B(N,1), A(M,2)*B(N,2), A(M,3)*B(N,3), A(M,1)*B(N,2)+ A(M,2)*B(N,1), A(M,1)*B(N,3)+A(M,3)*B(N,1), &
        A(M,2)*B(N,3)+A(M,3)*B(N,2) /)
        M=1
        N=2
        OUT(4,:)=(/A(M,1)*B(N,1), A(M,2)*B(N,2), A(M,3)*B(N,3), A(M,1)*B(N,2)+ A(M,2)*B(N,1), A(M,1)*B(N,3)+A(M,3)*B(N,1), &
        A(M,2)*B(N,3)+A(M,3)*B(N,2) /)
        M=3
        N=1
        OUT(5,:)=(/A(M,1)*B(N,1), A(M,2)*B(N,2), A(M,3)*B(N,3), A(M,1)*B(N,2)+ A(M,2)*B(N,1), A(M,1)*B(N,3)+A(M,3)*B(N,1), &
        A(M,2)*B(N,3)+A(M,3)*B(N,2) /)
        M=2
        N=3
        OUT(6,:)=(/A(M,1)*B(N,1), A(M,2)*B(N,2), A(M,3)*B(N,3), A(M,1)*B(N,2)+ A(M,2)*B(N,1), A(M,1)*B(N,3)+A(M,3)*B(N,1), &
        A(M,2)*B(N,3)+A(M,3)*B(N,2) /)
