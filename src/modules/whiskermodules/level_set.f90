...
subroutine update_ls_system(lssys,mesh,i_IMC,input_location, omp_run, pq, diffsys, ls_spatial)
  ...
  ! Subroutine variables
  integer                                   :: g, g_rows(2), nindecies, counter, i, inod
  ...
  do g = 1, lssys%ngrains
    ...
    if (ls_spatial) then
      call get_interface_spatial(lssys%line_ex(:, g_rows(1):g_rows(2)), lssys%line_ey(:, g_rows(1):g_rows(2)), &
      lssys%line_elms(:, g), lssys%line_seg(g), lssys%ed(:,:,g), mesh)
    else
      call get_interface(lssys%line_ex(:, g_rows(1):g_rows(2)), lssys%line_ey(:, g_rows(1):g_rows(2)), &
      lssys%line_elms(:, g), lssys%line_seg(g), lssys%ed(:,:,g), mesh)
    endif
  ...
  do g = 1,lssys%ngrains
    g_rows = [2*(g-1) + 1, 2*(g-1) + 2]
    if (ls_spatial) then
      call reinit_level_set_spatial(lssys%a(:,g),lssys%line_ex(:,g_rows(1):g_rows(2)),lssys%line_ey(:,g_rows(1):g_rows(2)), &
      lssys%closest_line(:,g),lssys%line_seg(g),mesh)
    else
      call reinit_level_set(lssys%a(:,g),lssys%line_ex(:,g_rows(1):g_rows(2)),lssys%line_ey(:,g_rows(1):g_rows(2)), &
      lssys%closest_line(:,g),lssys%line_seg(g),mesh)
    endif
