...
subroutine ComputeJint3(grain_mesh, line_ex, line_ey, nseg)
  ...
  ! Subroutine variables  
  type(sparse)          :: M
  integer, allocatable  :: lines(:), enod_m(:,:), bcvali(:)
  real(dp), allocatable :: bcval_u(:), bcval_coords(:,:)
  logical, allocatable  :: bool_seg(:)
  integer               :: nbnods, nbcval, nnods_bcseg, nseg_bcval, i, j, k, iseg, ierr
  real(dp)              :: bcval, x1, x2, y1, y2, Le, Me(2,2)
  logical               :: P1inbccoords, P2inbccoords
  ...
  do i = 1, nbcval
      ...
      do iseg = 1, nseg
          ...
          P1inbccoords = any((abs(bcval_coords(1,:) - x1) .lt. 1d-8) .and. (abs(bcval_coords(2,:) - y1) .lt. 1e-8))
          P2inbccoords = any((abs(bcval_coords(1,:) - x2) .lt. 1d-8) .and. (abs(bcval_coords(2,:) - y2) .lt. 1e-8))

          if (P1inbccoords .and. P1inbccoords) then
              bool_seg(iseg) = .true.
          end if
      end do
      ...
      ! --- Compute mass matrix ---
      allocate(enod_m(2,nseg_bcval),stat=ierr)
      do iseg=1,nseg_bcval
      ...
      ! Solve M*vnod = -r
      grain_mesh%jint(bcvali) = -grain_mesh%r(grain_mesh%bcnod(bcvali,1))
      call solveq(M, grain_mesh%jint(bcvali))
