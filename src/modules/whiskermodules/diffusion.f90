...
subroutine solve_diffusion_problem_global(diffsys,i_IMC,lssys,mesh,pq,input_location,omp_run)
  ...
  call generate_global_diffusion_mesh(diffsys, mesh, lssys, i_IMC)
...
subroutine generate_global_diffusion_mesh(diffsys, mesh, lssys, i_IMC)
  ...
  ! do g=1,lssys%ngrains
  g = 1
  call interpolate_scalar_mesh1_to_mesh2(mesh%enod, mesh%coord, lssys%ed(:,:,g), diffsys%enod,diffsys%coord, diffsys%ls)
  ! enddo
...
subroutine interpolate_scalar_mesh1_to_mesh2(mesh1_enod, mesh1_coord, mesh1_ed, mesh2_enod,mesh2_coord, mesh2_a)
  ...
  ! Intent inout
  real(dp), allocatable, intent(inout) :: mesh2_a(:)
  ...
  ! Allocate the field variable in mesh 2
  if (allocated(mesh2_a)) then
    deallocate(mesh2_a)
  endif
  allocate(mesh2_a(nnod2),stat=ierr)
