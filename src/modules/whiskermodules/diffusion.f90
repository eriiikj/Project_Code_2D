...
subroutine init_diffusion_system(diffsys, ngrains)
  ...
  real(dp) :: cu_cu, cu_imc, cu_sn, imc_cu, imc_imc, imc_sn, sn_cu, sn_imc, sn_sn
  real(dp) :: mu_cu_cu, mu_cu_imc, mu_cu_sn, mu_imc_cu, mu_imc_imc, mu_imc_sn, mu_sn_cu, mu_sn_imc, mu_sn_sn
  integer  :: ierr
  ...
  mu_cu_sn   = diffsys%thermo_parameterA(1)*(cu_sn  - diffsys%thermo_parameterxb(1)) + diffsys%thermo_parameterB(1)

  mu_imc_cu  = diffsys%thermo_parameterA(2)*(imc_cu  - diffsys%thermo_parameterxb(2)) + diffsys%thermo_parameterB(2)
  mu_imc_imc = diffsys%thermo_parameterA(2)*(imc_imc - diffsys%thermo_parameterxb(2)) + diffsys%thermo_parameterB(2)
  mu_imc_sn  = diffsys%thermo_parameterA(2)*(imc_sn  - diffsys%thermo_parameterxb(2)) + diffsys%thermo_parameterB(2)

  mu_sn_cu   = diffsys%thermo_parameterA(3)*(sn_cu  - diffsys%thermo_parameterxb(3)) + diffsys%thermo_parameterB(3)
  ...
  ! Row 2: IMC eq comps
  ! Row 3: Sn eq comps
  diffsys%eq_comp(1,:) = [mu_cu_cu , mu_cu_imc , mu_cu_sn]
  diffsys%eq_comp(2,:) = [mu_imc_cu , mu_imc_imc , mu_imc_sn]
  diffsys%eq_comp(3,:) = [mu_sn_cu , mu_sn_imc , mu_sn_sn]
...
subroutine solve_diffusion_problem_global(diffsys,i_IMC,lssys,mesh,pq,input_location,omp_run)
  ...
  call generate_global_diffusion_mesh(input_location, diffsys, mesh, lssys, i_IMC, omp_run)
...
subroutine generate_global_diffusion_mesh(input_location, diffsys, mesh, lssys, i_IMC, omp_run)
  ...
  print *, 'Interpolating from old to new mesh'  
  do g=1,lssys%ngrains
  ...
  print *, 'Reinitializing triangle mesh'
  do g = 1,lssys%ngrains
  ...
  enddo
  print *, 'Done reinitializing triangle mesh'
  ...
  print *, 'Extracting grain mesh data...'
  call chdir(diffusion_mesh_script_location)
...
subroutine diffusion_grain(diffsys, i_IMC, grain_mesh, g, input_location, lssys, omp_run)
  ! --- Routine for solving diffusion equation in grain ---
    ...
    call clock_time(t1, omp_run)
    call solve_diffusion_problem(grain_mesh)
    call clock_time(t2, omp_run)
...
subroutine compute_jint4(grain_mesh)
  ! --- Routine for ---
  ...
  integer                               :: nbseg, i, k, k1, k2, nod1, nod2, nods(2), ierr
  integer                               :: enod_l(2,size(grain_mesh%indLocalEdgBd)), enod_m(2,size(grain_mesh%indLocalEdgBd))
  real(dp)                              :: x1, x2, y1, y2, Le
  real(dp), dimension(2)                :: v1, v2
  real(dp), dimension(2, 2)             :: Me
  ...
  ! Compute enod_l
  do i = 1, nbseg
      k1          = grain_mesh%indLocalEdgBd(i)
      k2          = mod(k1, grain_mesh%nodel) + 1
      nod1        = grain_mesh%enod(k1,grain_mesh%indElemBd(i))
      nod2        = grain_mesh%enod(k2,grain_mesh%indElemBd(i))
      enod_l(:,i) = [nod1, nod2]
  end do

   ! Rewrite enod_m from unique values to 1:nnod
  call FindUniqueIntegers(reshape(enod_l,[nbseg*2]), oldNods)
  call bubble_sort_1D_integer(oldNods)
  do k=1,size(oldNods)
    where (enod_l .eq. oldNods(k)) enod_m = k
  enddo
  ...
  do i = 1, nbseg
      ...
      v1 = grain_mesh%coord(:,nod1)
      v2 = grain_mesh%coord(:,nod2)
...
subroutine mesh_reset(grain_mesh)
  implicit none
  ...
  if (allocated(grain_mesh%coord)) then
    deallocate(grain_mesh%coord)
    deallocate(grain_mesh%enod)
    deallocate(grain_mesh%a)
    deallocate(grain_mesh%r)
    deallocate(grain_mesh%bcnod)
    deallocate(grain_mesh%j_flux)
    deallocate(grain_mesh%ed)
  endif

  if (allocated(grain_mesh%bcval)) then
    deallocate(grain_mesh%bcval)
  endif

  if (allocated(grain_mesh%bcval_idx)) then
    deallocate(grain_mesh%bcval_idx)
  endif
...
subroutine solve_diffusion_problem(grain_mesh)
  implicit none
  ...
  grain_mesh%a = 0d0 ! fb
  call solveq(K, grain_mesh%a, grain_mesh%bcnod, grain_mesh%bcval, grain_mesh%dofnod)
...
subroutine ComputeJint3(grain_mesh, line_ex, line_ey, nseg)
  ! --- Routine for computing jint ---
  ...
  logical, allocatable  :: bool_seg(:)
  integer               :: nbnods, nbcval, nnods_bcseg, nseg_bcval, i, j, k, iseg, ierr, nod1(1), nod2(1), cc
  real(dp)              :: bcval, x1, x2, y1, y2, Le, Me(2,2)

  ! Number of boundary nods
  nbnods = size(grain_mesh%bcnod(:, 1))
  ...
  call FindUniqueREAL(grain_mesh%bcval, bcval_u)
  nbcval = size(bcval_u)
  ...
  do i = 1, nbcval
      bool_seg    = .false.
      bcval       = bcval_u(i)
      bcvali      = pack([(j, j=1,nbnods)], grain_mesh%bcval == bcval)
      nnods_bcseg = size(bcvali)
      allocate(bcval_coords(2,nnods_bcseg))
      bcval_coords = grain_mesh%coord(:,grain_mesh%bcnod(bcvali,1))
      allocate(enod_l(2,nnods_bcseg*4),stat=ierr)
      enod_l = 0
      cc = 0
      do iseg = 1, nseg
          x1 = line_ex(iseg, 1)
      ...
      ! Extract enod_m
      allocate(enod_m(2,cc),stat=ierr)
      enod_m = enod_l(:,1:cc)

      ! Rewrite enod_m from unique values to 1:nnod
      call FindUniqueIntegers(reshape(enod_m,[cc*2]), oldNods)
      call bubble_sort_1D_integer(oldNods)
      do k=1,size(oldNods)
        where (enod_l .eq. oldNods(k)) enod_m = k
      enddo

      ! Find lines with true values
      lines      = pack([(j, j=1, nseg)],bool_seg)
      nseg_bcval = size(lines)
      ...
      do k=1,nseg_bcval
        ...
        y1 = line_ey(iseg, 1)
        y2 = line_ey(iseg, 2)
        Le = norm2([(x2-x1), (y2-y1)]) ! Length of line segment

        ! Mass matrix
        Me(1,:) = [2d0, 1d0]
        Me(2,:) = [1d0, 2d0]
        Me      = Le/6d0*Me
        call assem(M,Me,enod_m(:,k),1)
      enddo

      ! Solve M*vnod = -r
      allocate(bb(size(bcvali)))      
      bb = -grain_mesh%r(grain_mesh%bcnod(bcvali,1))
      call solveq(M, bb)
      grain_mesh%jint(bcvali) = bb

      ! Deallocate
      deallocate(bb)
      deallocate(lines)
      deallocate(bcval_coords)
      deallocate(enod_l)
      deallocate(enod_m)
      deallocate(bcvali)
  end do
...
subroutine interpolate_scalar_mesh1_to_mesh2(mesh1_enod, mesh1_coord, mesh1_ed, mesh2_enod,mesh2_coord, mesh2_a)
  ...
  !$omp parallel private(inod,ie,x,y,pwt,xecoords,yecoords,x1,x2,x3,x4,y1,y2,y3,y4,pevals,xsi,eta,Nvec) shared(mesh2_a,mesh1_ed)
...
SUBROUTINE FindUniqueREAL(arr, uniqueArr)
  real(dp), INTENT(IN)               :: arr(:)      ! Input integer array
  real(dp), ALLOCATABLE, INTENT(OUT) :: uniqueArr(:)  ! Output array for unique integers
  real(dp), ALLOCATABLE              :: uniqueArr2(:)  ! Output array for unique integers
  INTEGER :: i, j, counter
  ...
  DO i = 2, size(arr)
    ! Check if the current element is unique
    IF (all(abs(uniqueArr2(1:counter) - arr(i)) .gt. 1d-10)) THEN
      ! Increment count and add the unique element to the output array
      counter = counter + 1
      uniqueArr2(counter) = arr(i)
    END IF
