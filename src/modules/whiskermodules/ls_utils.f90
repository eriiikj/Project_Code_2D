...
subroutine interface_reconstruction_spatial(lssys,mesh)
    ...
    !     lssys%line_coord([1:lssys%line_coordN(g)],g_cols(1):g_cols(2)) = unique_coord        
    ! enddo
...
subroutine lines_to_coord(line_ex, line_ey, N, unique_coord)
    ! --- Routine for finding unique coordinates in lines ---
    implicit none
    integer, intent(in)                :: N
    real(dp), intent(in)               :: line_ex(:,:), line_ey(:,:)
    real(dp)                           :: coord(2*N, 2)
    real(dp), allocatable, intent(out) :: unique_coord(:,:)
  
    ! Copy line_ex and line_ey to coord
    coord(1:N, 1)     = line_ex(:, 1)
    coord(N+1:2*N, 1) = line_ex(:, 2)
    coord(1:N, 2)     = line_ey(:, 1)
    coord(N+1:2*N, 2) = line_ey(:, 2)
  
    ! Find unique coordinates in coord
    call find_unique_coordinates(coord, 2*N, unique_coord)
...
subroutine sort_lines(line_ex,line_ey,tplines,line_seg,sep_lines,nsep_lines, mesh)
    ...
    sep_l_idx_ext = 0
    critical_length = 0.2d0*mesh%min_elm_width
...
subroutine interface_lseg_adjustment(line_ex,line_ey, nseg, tppoints, lseg)
    ...
    call remove_line_segments_noSorting(line_ex, line_ey, nseg, tppoints, lseg)
