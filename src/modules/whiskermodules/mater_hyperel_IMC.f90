...
subroutine neohooke1(stype,stress,ef,dg_ifstar)
  implicit none
  double precision                :: stress(:,:), ef(:,:), dg_ifstar(9)
  character(len=*)                :: stype
  integer                         :: gp, nr_gp
  ...
  do gp=1,nr_gp
    call neohooke2(stype,stress(:,gp), ef(:,gp), dg_ifstar)
  enddo
...
subroutine neohooke2(stype,stress,efe, dg_ifstar)
  implicit none
  double precision                :: stress(:), efe(:), dg_ifstar(9)
  character(len=*)                :: stype
  
  double precision                :: Fe(3,3), Ce(3,3), iCe(3,3), id(3,3), iFstar(3,3)
  double precision                :: Je, trCe, Sbar(3,3), S(3,3), invFe(3,3)
  

  ! --- Construct iiFstar ---
  iFstar = getF(dg_ifstar)
  ! iFstar      = 0d0 
  ! iFstar(1,1) = 1d0/(1d0 + IMC_eps_star)
  ! iFstar(2,2) = 1d0/(1d0 + IMC_eps_star)
  ! iFstar(3,3) = 1d0/(1d0 + IMC_eps_star)

  ! --- Construct Fe ---
  if (size(efe).eq.4) then
    Fe(1,:)=(/efe(1), efe(2), 0d0/)
    Fe(2,:)=(/efe(3), efe(4), 0d0/)
    Fe(3,:)=(/  0d0,   0d0, 1d0/)
  else
    Fe(1,:)=(/efe(1), efe(2), efe(3)/)
    Fe(2,:)=(/efe(4), efe(5), efe(6)/)
    Fe(3,:)=(/efe(7), efe(8), efe(9)/)
  endif


  ! --- Compute stress in intermediate configuration ---
  ...
  Sbar = Kmp/2d0*(Je**2d0-1d0)*iCe+Gmp*Je**(-2d0/3d0)*(Id-trCe/3d0*iCe)


  ! --- Compute second Piola-Kirchhoff stress tensor ---
  S = matmul(iFstar,matmul(Sbar,transpose(iFstar)))
...
subroutine dneohooke1(stype,D,ef, dg_ifstar)
  implicit none
  double precision                :: D(:,:,:), ef(:,:), dg_ifstar(9)
  character(len=*)                :: stype
  integer                         :: gp,nr_gp
  ...
  do gp=1,nr_gp
    call dneohooke2(stype,D(:,:,gp),ef(:,gp),dg_ifstar)
  enddo
...
subroutine dneohooke2(stype,D,efe,dg_ifstar)
  implicit none
  double precision                :: D(:,:), efe(:), dg_ifstar(9)
  character(len=*)                :: stype
  
  double precision                :: K, G
  double precision                :: Fe(3,3), Ce(3,3), iCe(3,3), Id(3,3), iFstar(3,3), eiFstar(9)
  double precision                :: Je, trCe, a1, a2, a3, Ds(21), Dbar(6,6), Dfull(6,6), M3(3,3)
  ...
  a1=K*Je**2d0+2d0/9d0*G*Je**(-2d0/3d0)*trCe
  a2=2d0*G/3d0*Je**(-2d0/3d0)
  a3=G/3d0*Je**(-2d0/3d0)*trCe-K/2d0*(Je**2d0-1d0)
  ...
  if (size(efe).eq.4) then
    ...
    !   Dsbar(el)=a1*iCe(i,jj)*iCe(kk,l)-a2*(Id(i,jj)*iCe(kk,l)+iCe(i,jj)*Id(kk,l)) &
    !    +a3*(iCe(i,kk)*iCe(jj,l)+iCe(i,l)*iCe(jj,kk))
    ! enddo


    ! ! Dbar(1,:)=(/Ds(1), Ds(2), Ds(3)/)
    ! ! Dbar(2,:)=(/Ds(2), Ds(4), Ds(5)/)
    ! ! Dbar(3,:)=(/Ds(3), Ds(5), Ds(6)/)
  ...
  ! Size 9
    ...
    do el=1,21
      ...
      Ds(el)=a1*iCe(i,jj)*iCe(kk,l)-a2*(Id(i,jj)*iCe(kk,l)+iCe(i,jj)*Id(kk,l)) &
       +a3*(iCe(i,kk)*iCe(jj,l)+iCe(i,l)*iCe(jj,kk))
    enddo

    Dbar(1,:)=(/Ds(1),  Ds(2),  Ds(3),  Ds(4),  Ds(5),  Ds(6)/)
    Dbar(2,:)=(/Ds(2),  Ds(7),  Ds(8),  Ds(9), Ds(10), Ds(11)/)
    Dbar(3,:)=(/Ds(3),  Ds(8), Ds(12), Ds(13), Ds(14), Ds(15)/)
    Dbar(4,:)=(/Ds(4),  Ds(9), Ds(13), Ds(16), Ds(17), Ds(18)/)
    Dbar(5,:)=(/Ds(5), Ds(10), Ds(14), Ds(17), Ds(19), Ds(20)/)
    Dbar(6,:)=(/Ds(6), Ds(11), Ds(15), Ds(18), Ds(20), Ds(21)/)

  
  endif 

  ! --- D is the pull back of Dbar with iiFstar ---
  call pushforward(Dfull,Dbar,dg_ifstar)
  ...
  D(1,:) = (/Dfull(1,1), Dfull(1,2), Dfull(1,4)/)
  D(2,:) = (/Dfull(2,1), Dfull(2,2), Dfull(2,4)/)
  D(3,:) = (/Dfull(4,1), Dfull(4,2), Dfull(4,4)/)
